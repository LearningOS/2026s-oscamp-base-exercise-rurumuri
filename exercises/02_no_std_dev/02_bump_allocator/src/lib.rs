...
unsafe impl GlobalAlloc for BumpAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        let next_current = self.next.load(Ordering::Relaxed);
        // 2. Align next up to layout.align()
        //    Hint: align_up(addr, align) = (addr + align - 1) & !(align - 1)
        let next_aligned = (next_current + layout.align() - 1) & !(layout.align() - 1);
        // 3. Compute allocation end = aligned + layout.size()
        let alloc_end = next_aligned + layout.size();
        ...
        match self.next.compare_exchange(next_current, alloc_end, Ordering::SeqCst, Ordering::Relaxed) {
            Ok(_) => next_aligned as *mut u8,
            Err(_) => self.alloc(layout), // retry on failure
        }
    }
