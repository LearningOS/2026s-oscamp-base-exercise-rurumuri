...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    // todo!()
    (
        thread::spawn(move || {a.iter().sum::<i32>()}).join().unwrap(),
        thread::spawn(move || {b.iter().sum::<i32>()}).join().unwrap()
    )
}
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    let sleeper_thread = builder.spawn(move || {
        thread::sleep(Duration::from_millis(ms));
        value
    });
    sleeper_thread.unwrap().join().unwrap()
}
