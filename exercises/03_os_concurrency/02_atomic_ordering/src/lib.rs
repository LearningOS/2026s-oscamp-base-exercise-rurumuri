...
impl OnceCell {
    ...
    pub fn init(&self, val: u32) -> bool {
        ...
        // todo!()
        let initialized_cur = self.initialized.load(Ordering::Relaxed);
        if !initialized_cur {
        ...
        false
        // match self.initialized.compare_exchange(initialized_cur, true, Ordering::AcqRel, Ordering::Acquire) {
        //     Ok(true) => {
        //         self.value.store(val, Ordering::Release);
        //         true
        //     },
